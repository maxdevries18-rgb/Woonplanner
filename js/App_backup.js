...
function App() {
  ...
  const [form,setForm]=useState({title:"",userIds:[],teamIds:[],category:DEFAULT_CATEGORIES[0].id,dayIndex:0,deadline:"hele_dag",notes:"",recurrenceType:"none",recurrenceDay:null,recurrenceEndDate:"",recurrenceLimit:""});
  ...
  async function saveTask(){
    ...
    const taskData = {
      ...
      notes:form.notes||"",
      recurrence_type:form.recurrenceType==="none"?null:form.recurrenceType,
      recurrence_day:form.recurrenceType==="none"?null:recDay,
      recurrence_end_date:form.recurrenceType!=="none" && form.recurrenceEndDate ? form.recurrenceEndDate : null,
      recurrence_limit:form.recurrenceType!=="none" && form.recurrenceLimit ? parseInt(form.recurrenceLimit) : null
    };
    ...
    try{
      if(editingTask&&!editingTask.isVirtual){
        ...
        if(targetId){
          ...
          const {data,error}=await supabase.from("tasks").insert([{...taskData, done:false, week_monday:currentMonday}]).select().single();
      ...
      setForm(f=>({...f,title:"",userIds:[],teamIds:[],dayIndex:selectedDay,deadline:"hele_dag",notes:"",recurrenceType:"none",recurrenceEndDate:"",recurrenceLimit:""}));
      await loadAll();
  ...
  function openEditModal(task){
    ...
    setForm({
      ...
      recurrenceLimit:taskToEdit.recurrenceLimit||""
    });
    setShowModal(true);
  }
      ...
      {showModal&&(
        <Modal title={editingTask?"✏️ Taak wijzigen":"➕ Nieuwe taak"} onClose={closeTaskModal} maxWidth={500}>
            ...
            <div style={{marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:form.recurrenceType!=="none"?10:0}}>
                ...
                <div style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:12,border:"1px solid rgba(255,255,255,0.08)"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    <div>
                      ...
                      <input type="date" value={form.recurrenceEndDate} onChange={e=>setForm(f=>({...f,recurrenceEndDate:e.target.value}))} style={{width:"100%",background:"#2a2a5a",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"7px 9px",color:"#fff",fontSize:12,outline:"none"}}/>
                    </div>
                    <div>
                      ...
                      <input type="number" placeholder="Bijv. 10" value={form.recurrenceLimit} onChange={e=>setForm(f=>({...f,recurrenceLimit:e.target.value}))} style={{width:"100%",background:"#2a2a5a",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"7px 9px",color:"#fff",fontSize:12,outline:"none"}}/>
                    </div>
                  </div>
